...
def create_cifar10_ae(input_file, batch_size=128):
    ...
    net.deconv3 = L.Deconvolution(net.relu2, convolution_param=dict(num_output=256, kernel_size=3, stride=2,
            pad=1, weight_filler=dict(type='gaussian', std=0.02)))
    net.relu3 = L.ReLU(net.deconv3, in_place=True)
    # 8x8

    net.deconv2 = L.Deconvolution(net.relu3, convolution_param=dict(num_output=256, kernel_size=3, stride=2,
            pad=1, weight_filler=dict(type='gaussian', std=0.02)))
    net.relu2 = L.ReLU(net.deconv2, in_place=True)
    # 16x16

    net.deconv1 = L.Deconvolution(net.relu2, convolution_param=dict(num_output=128, kernel_size=3, stride=2,
            pad=1, weight_filler=dict(type='gaussian', std=0.02)))
    net.relu1 = L.ReLU(net.deconv2, in_place=True)
    # 32x32

    net.conv_output = L.Convolution(net.relu1, num_output=64, kernel_size=3, stride=2,
            pad=1, weight_filler=dict(type='gaussian', std=0.02))
...
with open(output_file, 'w') as f:
    f.write(str(create_cifar10_ae(train_h5list_file)))